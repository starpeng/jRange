...
(function ($, window, document, undefined) {
    ...
    jRange.prototype = {
        ...
        init: function (node, options) {
            ...
            if (this.interval == 0) {
                console.log('jRange : interval must great than 0!, returning');
                //throw 'interval must great than 0!'
                return;
            }
        ...
        render: function () {
            ...
            if (!this.isSingle() && this.options.period > 0) {
                if (this.options.period > this.interval) {
                    console.log('jRange : period must less than interval, returning');
                    //throw 'period must less than interval!'
                    return;
                }

                this.period = this.options.period * this.domNode.width() / this.interval;
            } else {
                this.period = 0;
            }
        ...
        onChange: function (e, self, pointer, position) {
            ...
            if (self.isSingle()) {
                ...
                min = pointer.hasClass('high') ? (self.lowPointer.position().left + self.lowPointer.width() / 2 + self.period) : 0;
                max = pointer.hasClass('low') ? (self.highPointer.position().left + self.highPointer.width() / 2 - self.period) : self.domNode.width();
            }
